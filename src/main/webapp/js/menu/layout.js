...
//===========================工具类函数============================
//只允许输入数字
function checkNumber(e,callback) {
...
function checktxt(obj, txtId) {
    ...
    var mobils = $("#txtSmsContent").val();
	if(mobils.length<15){
		 var smsLength = Math.ceil(txtCount / 62);
		    $("#" + txtId).html("您已输入<b>" + txtCount + "/15</b>个字符，将以<b>" + smsLength + "</b>条短信扣取费用。<font color='red'>短信字数请大于15！</font>");
	}else{
		 var smsLength = Math.ceil(txtCount / 62);
...
function jsprint(msgtitle, url, msgcss, callback) {
    ...
    if(msgtitle!=null && msgtitle!="" && msgtitle!="noDialog"){
    	$.dialog.tips(msgtitle, 2, iconurl);
    }
    if (url == "back") {
        parent.frames["mainframe"].history.back(-1);
    }     
    else if (url != "") {    	
        ...
        $(formObj).Validform({
			...
			callback:function(data){
			   if(data)
			   {
				   ...
				   {
					   parent.jsprint(data.msg_title, data.url, data.result_type);
					   return
				   }
...
$.fn.initIgnoreajaxValidform = function () {
    var checkValidform = function (formObj) {
    	var validedform = $(formObj).Validform({
            tiptype: function (msg, o, cssctl) {
                /*msg：提示信息;
                o:{obj:*,type:*,curform:*}
                obj指向的是当前验证的表单元素（或表单对象）；
...
$.fn.ruleMultiPorp = function() {
	var multiPorp = function(parentObj){
		...
		parentObj.find(":checkbox").each(function(){
			...
			var newObj = $('<a href="javascript:;">' + parentObj.find('label').eq(indexNum).text() + '</a><i></i><input name='+parentObj.find('label').eq(indexNum).text()+' type="text" maxlength="255" id="txtLinkUrl"  class="input normal" />').appendTo(liObj); //查找对应Label创建选项
